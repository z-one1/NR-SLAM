                                                 ...
                                                 const float scale)
{
    ...
    for (int i = 0; i < keypoints.size(); ++i) {
        ...
        edge->calibration_ = current_frame.GetCalibration(); // 相机内参
        edge->landmark_world_ = landmark_positions[i].cast<double>(); // 固定地图点
